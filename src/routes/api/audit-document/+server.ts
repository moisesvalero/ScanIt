...
function inspectPdfSignature(buffer: Buffer): PdfSignatureCheck {
  ...
  const hasSigMarkers =
    ...
    raw.includes("/ByteRange[") ||
    raw.includes("/SubFilter");
  if (!hasSigMarkers) {
    return {
      hasSignature: false,
      status: "unsigned",
      reason: "No se detecta contenedor de firma digital en el PDF.",
    };
  }
  const matches = [
    ...raw.matchAll(/\/ByteRange\s*\[\s*(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s*\]/g),
  ];
  if (matches.length === 0) {
    return {
      hasSignature: true,
      status: "unknown",
      reason: "Firma detectada sin ByteRange parseable.",
    };
  }
  ...
  if (hasStructurallyValidRange && !hasPostSignedBytes) {
    return {
      ...
      reason: "Estructura de firma coherente (verificacion estructural).",
    };
  }
  ...
  if (hasStructurallyValidRange && hasPostSignedBytes) {
    return {
      ...
      status: "unknown",
      reason:
        "Firma con bytes fuera del rango principal (posible actualizacion incremental/LTV); requiere validacion PKI para confirmar.",
    };
  }
  ...
  if (hasImpossibleRange) {
    return {
      ...
      reason:
        "ByteRange inconsistente: posible alteracion del wrapper de firma.",
    };
  }
  ...
  return {
    hasSignature: true,
    status: "unknown",
    reason: "Firma detectada con estructura no concluyente.",
  };
}
...
async function parsePdf(buffer: Buffer) {
  ...
  try {
    ...
    try {
      ...
      totalPages =
        typeof infoResult?.total === "number" ? infoResult.total : null;
    } finally {
    ...
    const literalChunks = [...rawPdf.matchAll(/\((?:\\.|[^\\()]){2,}\)/g)]
      ...
      .filter((chunk) =>
        /[A-Za-zÀ-ÿ\u0400-\u04FF\u0600-\u06FF\u4E00-\u9FFF]/.test(chunk),
      );
    text = literalChunks.join(" ").replace(/\s+/g, " ").trim();
    totalPages = (rawPdf.match(/\/Type\s*\/Page\b/g) ?? []).length || null;
    info = {
      CreationDate: (rawPdf.match(/\/CreationDate\s*\(([^)]{4,})\)/)?.[1] ??
        null) as string | null,
      ModDate: (rawPdf.match(/\/ModDate\s*\(([^)]{4,})\)/)?.[1] ?? null) as
        | string
        | null,
      Creator: (rawPdf.match(/\/Creator\s*\(([^)]{2,})\)/)?.[1] ?? null) as
        | string
        | null,
      Producer: (rawPdf.match(/\/Producer\s*\(([^)]{2,})\)/)?.[1] ?? null) as
        | string
        | null,
    };
  }
  ...
  const hasDigitalSignature =
    rawPdf.includes("/Type/Sig") ||
    rawPdf.includes("/ByteRange[") ||
    rawPdf.includes("/SubFilter");
...
function isOfficialPdfSource(metadata: Record<string, unknown>) {
  const base =
    `${String(metadata.creator ?? "")} ${String(metadata.application ?? "")} ${String(metadata.producer ?? "")}`.toLowerCase();
...
function buildAnomalies(input: {
  ...
  metadataConsistency?: {
    docxAuthorMismatch: boolean;
    docxTimelineMismatch: boolean;
  };
  pdfSignature?: PdfSignatureCheck;
}) {
  const anomalies: Anomaly[] = [];
  const ratio =
    input.editingMinutes && input.editingMinutes > 0
      ? input.wordCount / input.editingMinutes
      : null;
  ...
  if (
    input.wordCount > 1000 &&
    input.editingMinutes !== null &&
    input.editingMinutes < 15
  ) {
    anomalies.push({
  ...
  if (
    app.includes("pandoc") ||
    producer.includes("pandoc") ||
    app.includes("google docs") ||
    producer.includes("google docs")
  ) {
    anomalies.push({
      code: "SUSPICIOUS_CREATOR_TOOL",
      severity: "amber",
      message:
        "Origen de creacion/exportacion atipico para entrega academica local (Pandoc/Google Docs).",
    });
  }
  ...
  if (
    input.uniformity &&
    input.uniformity.coefficient < 0.34 &&
    input.wordCount > 350
  ) {
    anomalies.push({
      ...
      severity: "amber",
      message:
        "Patron sintactico inusualmente uniforme; indicador compatible con generacion automatizada.",
    });
  }
  ...
  if (
    typeof input.lexicalDiversity === "number" &&
    input.lexicalDiversity < 0.22 &&
    input.wordCount > 260
  ) {
    anomalies.push({
      ...
      severity: "amber",
      message:
        "Diversidad lexical baja para la longitud observada; posible uniformidad estilistica excesiva.",
    });
  }
  if (
    input.extension === "docx" &&
    input.metadataConsistency?.docxAuthorMismatch
  ) {
    anomalies.push({
      code: "DOCX_AUTHOR_MISMATCH",
      severity: "amber",
      message:
        "Inconsistencia en metadatos internos DOCX: autor original y ultimo editor no coinciden.",
    });
  }
  if (
    input.extension === "docx" &&
    input.metadataConsistency?.docxTimelineMismatch
  ) {
    anomalies.push({
      code: "DOCX_TIMELINE_MISMATCH",
      severity: "red",
      message:
        "Inconsistencia temporal interna DOCX: saltos de fechas ilogicos entre metadatos.",
    });
  }
  if (
    input.extension === "pdf" &&
    input.pdfSignature?.status === "structural_tampered"
  ) {
    anomalies.push({
      code: "PDF_SIGNATURE_TAMPERED",
      severity: "red",
      message:
        "Wrapper de firma PDF inconsistente: posible manipulacion posterior al firmado.",
    });
  }
  ...
  let verdict: "integro" | "anomalias_detectadas" | "no_concluyente" =
    "no_concluyente";
...
function computeDocumentConfidence(input: {
  ...
  if (!input.timelineCreated || !input.timelineModified) {
    score -= 22;
    reasons.push(
      "Timeline incompleto: falta marca de creacion o modificacion.",
    );
  }
  ...
  if (input.editingMinutes === null && input.extension === "pdf") {
    score -= 6;
    reasons.push(
      "PDF final sin tiempo de edicion interno (normal en documentos cerrados).",
    );
  }
  if (input.ratioWordsPerMinute === null) {
    score -= 14;
    reasons.push(
      "No se pudo calcular ratio palabras/minuto por evidencia insuficiente.",
    );
  }
...
async function groqLinguisticAudit(
  text: string,
): Promise<GroqLinguisticVerdict | null> {
  ...
  if (jsonCandidate) {
    ...
    if (parsed) {
      const suspicionPercent = clamp01to100(
        Number(parsed?.suspicionPercent ?? parsed?.porcentaje ?? 0),
      );
      ...
      while (reasons.length < 3)
        reasons.push(
          "No concluyente: evidencia lingüística insuficiente en la muestra.",
        );
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  if (!(file instanceof File)) {
    return new Response(JSON.stringify({ error: "Archivo no valido." }), {
      status: 400,
    });
  }
  ...
  if (!extension) {
    return new Response(
      JSON.stringify({ error: "Formato no soportado. Usa .docx o .pdf." }),
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const parsed =
      extension === "docx" ? await parseDocx(buffer) : await parsePdf(buffer);
    ...
    const styleConsistencyIndex = Number(
      Math.max(
        ...
        Math.min(
          100,
          (((uniformity ? Math.max(0, 1 - uniformity.coefficient) : 0.42) * 60 +
            Math.min(1, entropy / 4.5) * 25 +
            diversity * 15) *
            100) /
            100,
        ),
    ...
    const docxAuthorMismatch =
      ...
      String(parsed.metadata.creator).trim().toLowerCase() !==
        String(parsed.metadata.lastModifiedBy).trim().toLowerCase();
    const createdTs = parsed.metadata.created
      ? Date.parse(String(parsed.metadata.created).replace(/^D:/, ""))
      : NaN;
    const modifiedTs = parsed.metadata.modified
      ? Date.parse(String(parsed.metadata.modified).replace(/^D:/, ""))
      : NaN;
    const docxTimelineMismatch =
      extension === "docx" &&
      Number.isFinite(createdTs) &&
      Number.isFinite(modifiedTs) &&
      modifiedTs < createdTs;
    const pdfSignature =
      extension === "pdf" ? inspectPdfSignature(buffer) : undefined;
    ...
    const safeModeEnabled =
      (process.env.SCANIT_SAFE_MODE ?? "").toLowerCase() === "true";
    ...
    if (safeModeEnabled && extension === "pdf") {
      linguisticAiStatus = {
        state: "omitted",
        reason:
          "Safe mode activo en produccion para PDF: capa lingüistica IA en pausa para priorizar estabilidad del servicio.",
      };
    } else if (!hasGroqKey) {
      linguisticAiStatus = {
        state: "omitted",
      ...
      linguisticAiStatus = {
        state: "omitted",
        reason:
          "Texto insuficiente o no extraible del documento (PDF escaneado, dibujo o imagen no textual).",
      };
    } else {
      try {
        ...
        if (isRateLimitError(e)) {
          linguisticAiStatus = {
            state: "error",
            reason:
              "Servicios IA temporalmente saturados; se continua con evidencia tecnica sin bloquear el analisis.",
          };
    ...
    if (
      parsed.wordCount > 1000 &&
      parsed.editingMinutes !== null &&
      parsed.editingMinutes < 20
    ) {
      mergedAnomalies.push({
    ...
    const officialPdfSource =
      extension === "pdf" ? isOfficialPdfSource(parsed.metadata) : false;
    const missingCriticalEvidence =
      extension === "docx"
        ? parsed.editingMinutes === null ||
          !parsed.metadata.created ||
          !parsed.metadata.modified ||
          parsed.wordCount < 180
    ...
    const rawPdfIntegroCap =
      process.env.SCANIT_PDF_INTEGRO_MAX_WORDS_IF_LINGUISTIC_ERROR;
    const pdfIntegroWordCap =
      rawPdfIntegroCap !== undefined && String(rawPdfIntegroCap).trim() !== ""
        ? Number(rawPdfIntegroCap)
        : 600;
    const integroWordCapIfLinguisticError = Number.isFinite(pdfIntegroWordCap)
      ? pdfIntegroWordCap
      : 600;
    if (
    ...
    if (
      extension === "pdf" &&
      linguisticAi &&
      linguisticAi.suspicionPercent >= 70
    ) {
    ...
    const pdfLinguisticDecisiveMin =
      rawDecisive !== undefined && String(rawDecisive).trim() !== ""
        ? Number(rawDecisive)
        : 90;
    const decisiveThreshold = Number.isFinite(pdfLinguisticDecisiveMin)
      ? pdfLinguisticDecisiveMin
      : 90;
    if (
      extension === "pdf" &&
      linguisticAi &&
    ...
    return new Response(
      JSON.stringify({
        ...
        metrics: {
          ...
          syntaxUniformityCoefficient: uniformity
            ? Number(uniformity.coefficient.toFixed(3))
            : null,
        ...
        evidenceCoverage: {
          timelineComplete: Boolean(
            parsed.metadata.created && parsed.metadata.modified,
          ),
          ...
          officialPdfSource,
          docxInternalMetadataConsistent: !(
            docxAuthorMismatch || docxTimelineMismatch
          ),
        ...
        ocrStatus:
          extension === "pdf" && parsed.wordCount < 40
            ? {
                state: "recommended",
                reason:
                  "PDF con poco texto extraible: se recomienda OCR sobre copia imagen para aumentar cobertura.",
              }
            : {
                state: "not_required",
                reason:
                  "Extraccion textual suficiente; OCR no necesario en esta fase.",
              },
        confidence,
        linguisticAi,
        linguisticAiStatus,
    ...
    const isRuntimePdfEngineIssue =
      extension === "pdf" &&
      (lower.includes("dommatrix is not defined") ||
        lower.includes("dommatrix"));

    const status =
      isEncrypted || isPdfStructureIssue || isRuntimePdfEngineIssue ? 422 : 500;

...
function normalizeOrigin(
  raw: unknown,
): "Origen: Dispositivo Digital (Captura)" | "Origen: Captura Óptica (Cámara)" {
...
async function runVisualPrecheck(
  groq: Groq,
  dataUrl: string,
): Promise<VisualPrecheck> {
  let response: any;
  try {
      ...
      messages: [
        ...
        },
        {
          ...
          content: [
            {
              type: "text",
              text: "Valida si esta imagen es evidencia documental académica apta para análisis forense.",
            },
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  if (!file.type.startsWith("image/")) {
    return new Response(
      JSON.stringify({ error: "Formato no soportado. Sube una imagen." }),
      { status: 400 },
    );
  }
    ...
    return new Response(
      JSON.stringify({
        ...
        status: {
          state: "omitted",
          reason:
            "Imagen demasiado pesada para analisis IA en tiempo real (max recomendado ~4.5MB).",
        } satisfies ImageAiStatus,
      }),
      { status: 200, headers: { "content-type": "application/json" } },
    );
  ...
  try {
          ...
          messages: [
            ...
            },
            {
              ...
              content: [
                {
                  type: "text",
                  text: "Analiza esta evidencia visual para detección forense de manipulación/sintético.",
                },
                { type: "image_url", image_url: { url: dataUrl } },
              ] as any,
            },
    ...
    if (!content) {
      return new Response(
        JSON.stringify({
          ...
          status: {
            state: "omitted",
            reason: "Groq no devolvio contenido para esta imagen.",
          } satisfies ImageAiStatus,
        }),
        { status: 200, headers: { "content-type": "application/json" } },
      );
    ...
    if (!parsed) {
      ...
      while (reasons.length < 3)
        reasons.push(
          "No concluyente: evidencia visual insuficiente en la muestra.",
        );

      const ocrTextSample =
        content.match(/ocrTextSample\s*[:=]\s*["']([\s\S]*?)["']/)?.[1] ?? "";
      const ocrEstimatedCharsStr = content.match(
        /ocrEstimatedChars\s*[:=]\s*(\d+)/i,
      )?.[1];
      const ocrEstimatedChars = ocrEstimatedCharsStr
        ? Math.max(0, Math.round(Number(ocrEstimatedCharsStr)))
        : ocrTextSample.length;
      const styleConsistencyStr = content.match(
        /styleConsistency\s*[:=]\s*(\d{1,3})/i,
      )?.[1];
      const styleConsistency = styleConsistencyStr
        ? clamp01to100(Number(styleConsistencyStr))
        : 50;
    ...
    const ocrTextSample = String(parsed?.ocrTextSample ?? "")
      .trim()
      .slice(0, 240);
    const ocrEstimatedCharsRaw = Number(
      parsed?.ocrEstimatedChars ?? ocrTextSample.length ?? 0,
    );
    const ocrEstimatedChars = Number.isFinite(ocrEstimatedCharsRaw)
      ? Math.max(0, Math.round(ocrEstimatedCharsRaw))
      : 0;
    const styleConsistency = clamp01to100(
      Number(parsed?.styleConsistency ?? 50),
    );
      ...
      return new Response(
        JSON.stringify({
          ...
          status: {
            state: "error",
            reason:
              "Servicios IA temporalmente saturados; se continua sin bloquear la certificacion visual.",
          } satisfies ImageAiStatus,
        }),
        { status: 200, headers: { "content-type": "application/json" } },
      );
    ...
    return new Response(
      JSON.stringify({
        ...
        status: {
          state: "error",
          reason: `Fallo en Groq Vision: ${msg.slice(0, 140)}`,
        } satisfies ImageAiStatus,
      }),
      { status: 200, headers: { "content-type": "application/json" } },
    );

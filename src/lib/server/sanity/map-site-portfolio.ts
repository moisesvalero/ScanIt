...
export function mapSanitySitePortfolio(
  ...
  if (!raw) {
    const base = {
      ...
      seo: {
        ...defaults.seo,
        ogImage: absolutizeOgImage(defaults.seo.ogImage, ctx.baseUrl),
      },
    };
  ...
  const mapped: SitePortfolioContent = {
    header: {
      ...
      navItems: Array.isArray(header?.navItems)
        ? (header?.navItems as SitePortfolioContent["header"]["navItems"])
    ...
    seo: {
      ...
      ogImage: absolutizeOgImage(
        asString(seo?.ogImage, defaults.seo.ogImage),
        ctx.baseUrl,
      ),
      twitterCard:
    ...
    hero: {
      ...
      ctaPrimaryLabel:
        asStringOpt(hero?.ctaPrimaryLabel) ?? defaults.hero.ctaPrimaryLabel,
      careerCtaLabel:
        asStringOpt(hero?.careerCtaLabel) ?? defaults.hero.careerCtaLabel,
    },
    about: {
      imageSrc:
        imageUrl(ctx.projectId, ctx.dataset, about?.image, 800) ||
        asString(about?.imageSrc, defaults.about.imageSrc),

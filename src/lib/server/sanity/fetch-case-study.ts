...
import {
  mapSanityRowToCaseStudy,
  type SanityCaseStudyRow,
} from "./map-sanity-case-study";
...
export async function fetchCaseStudyFromSanity(
  ...
  try {
    const row = await client.fetch<SanityCaseStudyRow | null>(
    ...
    if (
      typeof row.heroTag !== "string" ||
      typeof row.heroDescription !== "string"
    ) {
      return undefined;
    }

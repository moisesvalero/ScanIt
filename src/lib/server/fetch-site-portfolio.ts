...
} from "./sanity/get-server-client";
import { mapSanitySitePortfolio } from "./sanity/map-site-portfolio";

function defaultBaseUrl(): string {
  return new URL(publicEnv.PUBLIC_SITE_URL || "http://localhost:5173")
    .toString()
    .replace(/\/$/, "");
}

export async function fetchSitePortfolio(
  ...
  try {
    ...
    console.warn(
      "[portfolio] Sanity unavailable, using local defaults.",
      error,
    );
    return mapSanitySitePortfolio(null, sitePortfolioDefaults, {
      projectId: cfg.projectId,

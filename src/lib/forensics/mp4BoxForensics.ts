 ...
 */
export function scanMp4Container(
  buffer: ArrayBuffer,
  ...
  if (!ftypBox) {
    notes.push(
      "No se encontró box ftyp en el tramo analizado (contenedor no estándar o corrupto).",
    );
    return {
      applicable: true,
      fakeScore0to100: 78,
  ...
  const parsed = parseFtyp(
    dv,
    ftypBox.offset,
    Math.min(ftypBox.size, scanLen - ftypBox.offset),
  );
  ...
  if (ftypBox.offset !== 0) {
    notes.push(
      "ftyp no está al inicio del archivo (posible prefijo / empaquetado atípico).",
    );
    score += 35;
  }
  ...
  if (major && SUSPECT_MAJOR.has(major.trim())) {
    notes.push(
      `Marca mayor "${major}" asociada a fragmentación DASH / reempaquetado (no típico de captura directa).`,
    );
    score += 42;
  }
  ...
  if (!captureHint && major) {
    notes.push(
      "ftyp sin marcas compatibles habituales de captura móvil/cámara (posible reencode o muxer genérico).",
    );
    score += 28;
  }
  ...
  if (idxFree === 0 && boxes[0] && boxes[0].size > 512 * 1024) {
    notes.push(
      'Box "free" inicial muy grande (posible padding de herramienta).',
    );
    score += 18;
  }
  if (idxMdat >= 0 && idxMoov >= 0 && idxMdat < idxMoov) {
    notes.push(
      "mdat antes que moov (fast start inverso o mux atípico; no implica fake por sí solo).",
    );
    score += 12;
  }
  ...
  if (idxMoov < 0 && !order.includes("moof")) {
    notes.push(
      "No aparece moov en la cabecera analizada (puede ser fragmentado u offline parcial).",
    );
    score += 22;
  }

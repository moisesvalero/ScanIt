...
import type {
  ...
  ForensicInputs,
  SpecialistVote,
} from "../src/lib/ensemble/EnsembleManager.ts";
import { EnsembleManager } from "../src/lib/ensemble/EnsembleManager.ts";
import type {
  ContainerEnsembleInputs,
  RppgEnsembleInputs,
} from "../src/lib/forensics/AdvancedForensicSuite.ts";

const ROOT = process.cwd();
const DIRS = [
  path.join(ROOT, "tests", "dataset", "reales"),
  path.join(ROOT, "tests", "dataset", "fakes"),
];
...
function metadataFromWarnings(warnings: string[]) {
  ...
  return {
    thirdParty: w.some((s) =>
      /Software de Terceros|Terceros detected/i.test(s),
    ),
    originNoVerify: w.some((s) => /Origen Digital No Verificado/i.test(s)),
    missingCameraMeta: w.some((s) =>
      /Metadatos de Cámara Incompletos/i.test(s),
    ),
  };
}

function lowLevelForensicFromFeatures(
  f: Record<string, unknown>,
): Partial<ForensicInputs> {
  const o: Partial<ForensicInputs> = {};
  if (
    f.prnuResidualRisk0to100 != null &&
    Number.isFinite(Number(f.prnuResidualRisk0to100))
  ) {
    o.prnuResidualRisk0to100 = Number(f.prnuResidualRisk0to100);
  }
  if (
    f.dctDoubleQuantRisk0to100 != null &&
    Number.isFinite(Number(f.dctDoubleQuantRisk0to100))
  ) {
...
function advancedFromFeatures(
  feats: Record<string, unknown> | null | undefined,
) {
  ...
  if (adv.rppg && typeof adv.rppg === "object")
    out.rppg = adv.rppg as RppgEnsembleInputs;
  if (adv.container && typeof adv.container === "object")
    out.container = adv.container as ContainerEnsembleInputs;
...
for (const dir of DIRS) {
  ...
  for (const name of fs.readdirSync(dir)) {
    ...
    if (IMAGE_RE.test(name) && kind === "image" && feats?.forensic) {
      ...
      console.log(
        path.relative(ROOT, sidecarPath),
        "->",
        riskScore,
        full.verdict,
      );
    ...
    if (VIDEO_RE.test(name) && kind === "video") {
      ...
      if (!vf && IMPUTE_VIDEO) {
        ...
        videoImputed = true;
        console.warn(
          "impute video features:",
          path.relative(ROOT, sidecarPath),
          fromFakeDir ? "FAKE" : "REAL",
        );
      }
      if (!vf?.forensic || !vf?.biometric) {
        console.warn(
          "skip video (sin features; usa captura o --impute-video):",
          path.relative(ROOT, sidecarPath),
        );
        nSkipVid += 1;
        continue;
      ...
      if (forensics.videoPolishedRoiSyntheticHint)
        bio.videoPolishedRoiSyntheticHint = true;

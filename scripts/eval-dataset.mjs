...
const FETCH_ATTEMPTS = Math.max(1, Number(process.env.SCANIT_EVAL_FETCH_ATTEMPTS || 6));
const ALLOWED_EXT = new Set(['.pdf', '.docx']);
const MAX_PER_CLASS = Number(process.env.SCANIT_EVAL_MAX_PER_CLASS || 0);
const PARTIAL_EVERY = Math.max(1, Number(process.env.SCANIT_EVAL_PARTIAL_EVERY || 10));
...
async function main() {
  ...
  const partialDetailsPath = path.join(REPORT_DIR, `eval-partial-${stamp}.json`);
  const partialMetricsPath = path.join(REPORT_DIR, `eval-partial-metrics-${stamp}.json`);
  ...
  for (const item of items) {
    ...
    if (rows.length % PARTIAL_EVERY === 0) {
      ...
      await fs.writeFile(partialDetailsPath, JSON.stringify(rows, null, 2), 'utf8');
      await fs.writeFile(partialMetricsPath, JSON.stringify(partialMetrics, null, 2), 'utf8');
    }
  }

...
async function captureOne(page, filePath, sidecarPath) {
  ...
  if (!payload)
    throw new Error("No payload returned from __kronosGetExportPayload()");
...
async function run() {
  ...
  await page.waitForSelector('[data-testid="kronos-file-input"]', {
    timeout: 120000,
  });
